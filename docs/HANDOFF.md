...
- **`main`**: tudo do resumo acima mesclado e pushado. Suíte 407/407, typecheck limpo.
- **`config/companies.yaml`**: 17 empresas (era 2 no início da sessão anterior).
...
1. **Decisão de arquitetura do operador, não deste HANDOFF**: filtrar-no-pré-insert vs.
   inserir-tudo (dado novo no item 4 do resumo), e se/quando abrir Fase B — Solides (6 ocorrências
   no ranking) ou Greenhouse (1, mas API estável de verdade, não scrape). Nenhum dos dois
   implementado.
